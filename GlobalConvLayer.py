...
### TNN Gloabl Convolution Column Layer ###
## This class models all the columns in a single layer based on the input size, kernel (or RF) size and stride.
## Unlike a normal TNN layer, each RF is run through the same column, instead of one column per RF.
...
class GlobalConvLayer(nn.Module):
    def __init__(self, inputsize, rfsize, stride, nprev, q, theta, wres=3, w_init="half", ntype="rnl", device="cpu"):
        ...
        # self.rows * self.cols gives the total number of RFs (equivalently the total number of TNN columns) in
        # the current layer.
        ...
        # number of synapses per neuron per column
        ...
        if w_init == "zero":
            self.weights: torch.Tensor = nn.Parameter(torch.zeros(self.q, self.p), requires_grad=False)
        elif w_init == "half":
            self.weights: torch.Tensor = nn.Parameter((self.wmax / 2) * torch.ones(self.q, self.p), requires_grad=False)
        elif w_init == "uniform":
            self.weights: torch.Tensor = nn.Parameter(
                torch.randint(low=0, high=self.wmax + 1, size=(self.q, self.p)).type(torch.FloatTensor),
                requires_grad=False)
        elif w_init == "normal":
            self.weights: torch.Tensor = nn.Parameter(
                torch.round(((self.wmax + 1) / 2 + torch.randn(self.q, self.p)).clamp_(0, self.wmax)),
                requires_grad=False)
        ...
        self.stdp = STDP_Global(wres, self.rows, self.cols, self.num, self.p, device)

        # Initialize miscellaneous variables here.
        self.constidx: torch.Tensor = torch.arange(self.num).unsqueeze(1).repeat(1, self.p).to(device)
        self.constdec: torch.Tensor = torch.arange(self.p, 0, -1).repeat(self.num, 1).to(device)
        self.constidx2: torch.Tensor = torch.arange(self.num).to(device)
        self.ones: torch.Tensor = torch.ones(self.rows * self.cols, self.q).to(device)
    ...
    def __call__(self, data):
        ...
        # duplicate the weights over all of the receptive fields
        eff_weights = self.weights.unsqueeze(0).repeat(self.rows * self.cols, 1, 1)
        eff_weights = eff_weights.reshape(self.num, self.p)
        ...
        if self.ntype == "rnl":
            ...
            simTimes: torch.Tensor = ec_times.unsqueeze(1).expand(-1, self.p).unsqueeze(2).expand(-1, -1, self.wmax) + self.rnlTimes

            # get the weight of each neuron and the time each synapse fired into the same format
            timedWeights: torch.Tensor = torch.round(sorted_weights).unsqueeze(2).expand(-1, -1, self.wmax)
            timedFirings: torch.Tensor = sorted_times.unsqueeze(2).expand(-1, -1, self.wmax)

            # compute the rnl response over these times parametrically
            linearRnl: torch.Tensor = simTimes - timedFirings + 1
            clampedRnl: torch.Tensor = torch.where(simTimes < timedFirings, self.scalar0, linearRnl)
            rnl: torch.Tensor = torch.where(simTimes >= timedFirings + timedWeights, timedWeights, clampedRnl)

            # sum along the synapse axis
            timedPot: torch.Tensor = rnl.sum(axis=1)
...
class STDP_Global():
    def __init__(self, wres, rows, cols, num, p, device):
        self.rows = rows
        self.cols = cols
        self.num = num
        self.p = p
    ...
    def __call__(self, ec_in, li_out, weights, capture, search, backoff):
        ...
        # update as if we independently ran STDP on each receptive field in series (but without updating weights in between)

        # duplicate the weights over all of the receptive fields
        eff_weights = weights.unsqueeze(0).repeat(self.rows * self.cols, 1, 1)
        eff_weights = eff_weights.reshape(self.num, self.p)

        lt: torch.Tensor = ec_in <= li_out
        in_inf: torch.Tensor = ec_in == float('inf')
        out_inf: torch.Tensor = li_out == float('inf')

        # need to avoid case1 when out is inf
        case1: torch.Tensor = lt & ~out_inf
        case24: torch.Tensor = ~lt
        case3: torch.Tensor = ~in_inf & out_inf

        # for the bimodalization
        overHalf = eff_weights >= (self.wmax / 2)

        # make the updates for all the weights
        update = torch.zeros_like(eff_weights, device=self.device)
        ...
        # apply the update
        update = update.reshape(self.rows * self.cols, weights.shape[0], weights.shape[1])
        smallUpd = update.sum(axis=0)
        weights += smallUpd
