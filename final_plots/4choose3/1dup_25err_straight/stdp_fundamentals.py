...
def main():
    ...
    # duplicate the first channel to the first-er channel
    # this means when we try every possible sample, it looks like for diagonals
    d = np.concatenate((d[:, :, 0].reshape(*d.shape[:-1], 1), d), axis=2)
    d = torch.as_tensor(d)
    ...
    for epoch in range(nepochs):
        ...
        for i in rng.permutation(d.shape[0]):
            td = d[i].clone()
            # add noise to the duplicated channel
            if rng.random() < 0.25:
                # choose a different spike location
                pos = torch.argmin(td[:, :, 0].flatten())
                offs = rng.integers(1, nsamps - 1)
                pos = (pos + offs) % nsamps
                x = pos // nsampsSide
                y = pos % nsampsSide
                td[:, :, 0] = torch.ones((nsampsSide, nsampsSide)) * float('inf')
                td[x, y, 0] = 0
                if doposneg:
                    td[:, :, 1] = torch.zeros((nsampsSide, nsampsSide))
                    td[x, y, 1] = float('inf')
    ...
    d = kOfN(nchans + 1, nsamps)  # samples x n (elements) x k (channels)
    d = torch.as_tensor(d)
    # turn it into samples x side x side x k
    d = torch.reshape(d, (d.shape[0], nsampsSide, nsampsSide, -1))
    if doposneg:
